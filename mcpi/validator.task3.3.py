...
def equal_with_error(val, reference):
	return abs(1. - (val / float(reference))) < pi_error

def PiAllScenarios_Test(npoints, threads, expected):

	return TestGroup(
		AbortingTestGroup(*(
			ExecutionTest("./mcpi", [npoints, threads, strategy], [expected], compare=equal_with_error, key=lambda l: next(map(float, l)))
				for strategy in range(1,strategy_count+1)
				)
			),
		EmptyTestLine(),
	)
...
def main(cd=None):
	...
	tests = AbortingTestGroup(
		EmptyTestLine("[INFO] We expect your random number generator to be deterministic.\nBut we still allow you to be %.1f%% off.\n" % (pi_error * 100)),
		PiAllScenarios_Test(100,     1, "2.880000000000000"),
		PiAllScenarios_Test(100,     4, "3.160000000000000"),
		PiAllScenarios_Test(100000, 16, "3.146200000000000"),
		EmptyTestLine("[OKAY]")
	)
